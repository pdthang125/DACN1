...
import { CalendarIcon, ClockIcon, InfoIcon, ShieldCheckIcon } from "lucide-react";
  ...
  const handleBookAppointment = async () => {
    ...
    bookAppointmentMutation.mutate(
      ...
      },
      {
        ...
        onError: (error) => toast.error(`Lỗi đặt lịch: ${error.message}`),
      }
    );
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-3">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-white/10 border border-white/20 rounded-full text-[10px] font-black uppercase tracking-widest text-white/90">
                <CalendarIcon className="size-3" /> Hệ thống đặt lịch thông minh
              </div>
              <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight">
                Đặt Lịch <span className="text-blue-200">Khám Nha Khoa</span>
              </h1>
              <p className="text-blue-100 font-medium text-lg max-w-xl">
                Lựa chọn bác sĩ giỏi và khung giờ phù hợp nhất với bạn chỉ trong vài bước đơn giản.
              </p>
            </div>
            ...
            <div className="flex items-center gap-4 bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-4">
              <div className="flex flex-col items-center gap-1 border-r border-white/10 pr-4">
                <span className="text-xs text-blue-200 font-bold uppercase">Bác sĩ</span>
                <span className="text-2xl font-black text-white">15+</span>
              </div>
              <div className="flex flex-col items-center gap-1">
                <span className="text-xs text-blue-200 font-bold uppercase">Hài lòng</span>
      ...
      <div className="max-w-7xl mx-auto px-6 -mt-12 relative z-20 pb-20">
        ...
        {/* User's existing appointments section with more "Depth" */}
        {userAppointments.length > 0 && (
          <div className="mt-16">
            <div className="flex items-center gap-3 mb-8">
              ...
              <div>
                <h2 className="text-2xl font-black text-slate-900 tracking-tight">Lịch hẹn của bạn</h2>
                <p className="text-sm text-slate-500 font-medium">Theo dõi các buổi khám sắp tới</p>
              </div>
            </div>

            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {userAppointments.map((appointment) => (
                <div key={appointment.id} className="group bg-white border border-slate-100 rounded-3xl p-6 shadow-sm hover:shadow-xl hover:shadow-blue-100 transition-all duration-300 hover:-translate-y-1">
                  <div className="flex items-center gap-4 mb-6">
                    <div className="size-14 rounded-2xl overflow-hidden border-2 border-blue-50 shadow-md">
                      <img
                        src={appointment.doctorImageUrl || "https://github.com/shadcn.png"}
                        alt={appointment.doctorName}
                        className="size-full object-cover"
                      />
                    </div>
                    <div>
                      <p className="font-black text-slate-900 text-base">{appointment.doctorName}</p>
                  ...
                  <div className="space-y-3 bg-slate-50 rounded-2xl p-4">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-slate-400 font-medium">Ngày khám</span>
                      <span className="text-slate-900 font-black">{format(new Date(appointment.date), "dd/MM/yyyy")}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-slate-400 font-medium">Giờ khám</span>
      ...
      {bookedAppointment && (
        <AppointmentConfirmationModal
          ...
          appointmentDetails={{
            ...
            appointmentDate: format(new Date(bookedAppointment.date), "dd/MM/yyyy"),
